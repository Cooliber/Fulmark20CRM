...
export class HvacConfigService {
  private readonly logger = new Logger(HvacConfigService.name);
  ...
  getHvacApiConfig(): HvacApiConfig {
    return {
      url:
        this.configService.get('HVAC_API_URL') || 'http://localhost:8000',
      apiKey: this.configService.get('HVAC_API_KEY') || '',
      version: this.configService.get('HVAC_API_VERSION') || 'v1',
  ...
  getWeaviateConfig(): WeaviateConfig {
    return {
      host: this.configService.get('WEAVIATE_HOST') || 'localhost',
  ...
  getBielikConfig(): BielikConfig {
    return {
      host: this.configService.get('BIELIK_HOST') || 'localhost',
      port: this.getNumberConfig('BIELIK_PORT', 8123),
      apiKey: this.configService.get('BIELIK_API_KEY') || '',
      modelName:
        this.configService.get('BIELIK_MODEL_NAME') || 'bielik-v3-4.5b',
  ...
  getHvacBusinessConfig(): HvacBusinessConfig {
    return {
      companyName:
        this.configService.get('COMPANY_NAME') || 'Fulmark HVAC',
      companyEmail:
        this.configService.get('COMPANY_EMAIL') || 'info@fulmark.pl',
      companyNip: this.configService.get('COMPANY_NIP'),
      companyRegon: this.configService.get('COMPANY_REGON'),
      timezone: this.configService.get('TIMEZONE') || 'Europe/Warsaw',
      locale: this.configService.get('LOCALE') || 'pl_PL.UTF-8',
      currency: this.configService.get('LOCALIZATION_CURRENCY') || 'PLN',
    };
  }
  ...
  getHvacFeatureFlags(): HvacFeatureFlags {
    return {
      ...
      semanticSearch: this.getBooleanConfig(
        'FEATURE_HVAC_SEMANTIC_SEARCH',
        true,
      ),
  ...
  getHvacServiceEndpoints(): HvacServiceEndpoints {
    ...
    return {
      customers:
        this.configService.get('HVAC_CUSTOMER_SERVICE_URL') ||
        `${baseUrl}/api/${version}/customers`,
      tickets:
        this.configService.get('HVAC_TICKET_SERVICE_URL') ||
        `${baseUrl}/api/${version}/tickets`,
      equipment:
        this.configService.get('HVAC_EQUIPMENT_SERVICE_URL') ||
        `${baseUrl}/api/${version}/equipment`,
      maintenance:
        this.configService.get('HVAC_MAINTENANCE_SERVICE_URL') ||
        `${baseUrl}/api/${version}/maintenance`,
      semanticSearch:
        this.configService.get('HVAC_SEMANTIC_SEARCH_URL') ||
  ...
  getPolishComplianceConfig() {
    return {
      ...
      anonymizationEnabled: this.getBooleanConfig(
        'GDPR_ANONYMIZATION_ENABLED',
        true,
      ),
  ...
  getCacheConfig() {
    return {
      ...
      customerProfileTtl: this.getNumberConfig(
        'CACHE_TTL_CUSTOMER_PROFILE',
        1800,
      ),
    };
  }
  ...
  getPerformanceConfig() {
    return {
      ...
      maxConcurrentRequests: this.getNumberConfig(
  ...
  validateHvacConfiguration(): { isValid: boolean; errors: string[] } {
    ...
    if (bielik.apiKey || bielik.host !== 'localhost') {
      // Example condition to check if Bielik is actively configured
      if (!bielik.host) {
        errors.push(
          'BIELIK_HOST is required if Bielik integration is configured.',
        );
      }
      ...
      if (!bielik.apiKey) {
        errors.push(
          'BIELIK_API_KEY is required if Bielik integration is configured.',
        );
      }
    }
    ...
    if (
      business.companyNip &&
      !/^\d{10}$/.test(business.companyNip.replace(/-/g, ''))
    ) {
  ...
  async checkHvacServicesHealth(): Promise<{
    [service: string]: { healthy: boolean; message?: string };
  }> {
    const results: {
      [service: string]: { healthy: boolean; message?: string };
    } = {};
    ...
    try {
      ...
      results.hvacApi = {
        healthy: true,
        message: 'Placeholder: Health check not implemented',
      };
    } catch (e) {
    ...
    this.logger.log(
      'Performed HVAC services health check (placeholders).',
      results,
    );
