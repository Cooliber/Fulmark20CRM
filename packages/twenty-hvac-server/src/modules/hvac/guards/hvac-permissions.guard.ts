...
// import { PermissionsService } from '../../../../../../twenty-server/src/engine/metadata-modules/permissions/permissions.service';
// Note: PermissionsService is not available in separated HVAC server
...
export const RequireHvacPermissions = (...permissions: HvacPermissionType[]) =>
  Reflector.createDecorator<HvacPermissionType[]>({ key: HVAC_PERMISSIONS_KEY });
...
export class HvacPermissionsGuard implements CanActivate {
  constructor(
    ...
    // private readonly permissionsService: PermissionsService, // Not available in separated server
  ...
  private async checkHvacPermissions(
    ...
    try {
      // Note: Using simplified permission check since permissionsService is not available
      const hasAdminPermission = true; // TODO: Implement proper permission checking when available
