...
export enum HvacErrorType {
  ...
  // Additional error types referenced in filters
...
export enum HvacErrorCategory {
  ...
  // Additional categories referenced in filters
...
export class HvacError extends Error {
  ...
  constructor(details: HvacErrorDetails) {
    ...
    this.name = 'HvacError';
    this.id = details.metadata.correlationId;
    ...
    this.timestamp = new Date(details.metadata.timestamp);
    this.polishMessage = details.polishContext?.userFriendlyMessage || details.message;
...
export class HvacErrorHandlerService {
  ...
  /**
   * Handle and process HVAC errors
   */
  handleError(error: HvacError): void {
    // Log the error
    this.logger.error(`HVAC Error [${error.correlationId}]: ${error.message}`, {
      type: error.type,
      severity: error.severity,
      category: error.category,
      metadata: error.metadata,
    ...
    // Handle recovery if applicable
    if (error.recoveryStrategy.retryable) {
      const attempts = this.recoveryAttempts.get(error.correlationId) || 0;
      if (attempts < error.recoveryStrategy.maxRetries) {
        this.recoveryAttempts.set(error.correlationId, attempts + 1);
        this.logger.log(`Scheduling retry ${attempts + 1}/${error.recoveryStrategy.maxRetries} for error ${error.correlationId}`);
      }
    }

    // Send notifications if required
    if (error.recoveryStrategy.notificationRequired) {
      this.logger.warn(`Notification required for error: ${error.correlationId}`);
    }

    // Escalate if required
    if (error.recoveryStrategy.escalationRequired) {
      this.logger.error(`Escalation required for error: ${error.correlationId}`);
    }
  }
}
