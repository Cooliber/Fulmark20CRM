...
export class HvacDataSyncService {
  ...
  private async syncEquipment(metrics: SyncMetrics): Promise<void> {
    try {
      ...
      const equipmentResponse = await this.hvacApiService.getEquipment(undefined, 100, 0);
      metrics.equipment.total = equipmentResponse.total;

      for (const item of equipmentResponse.equipment) {
        try {
