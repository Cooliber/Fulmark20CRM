...
export class HvacConfigService {
  ...
  getHvacApiConfig(): HvacApiConfig {
    return {
      url:
        this.twentyConfigService.get('HVAC_API_URL') || 'http://localhost:8000',
  ...
  getBielikConfig(): BielikConfig {
    return {
      ...
      modelName:
        this.twentyConfigService.get('BIELIK_MODEL_NAME') || 'bielik-v3-4.5b',
  ...
  getHvacBusinessConfig(): HvacBusinessConfig {
    return {
      companyName:
        this.twentyConfigService.get('COMPANY_NAME') || 'Fulmark HVAC',
      companyEmail:
        this.twentyConfigService.get('COMPANY_EMAIL') || 'info@fulmark.pl',
  ...
  getHvacServiceEndpoints(): HvacServiceEndpoints {
    ...
    return {
      customers:
        this.twentyConfigService.get('HVAC_CUSTOMER_SERVICE_URL') ||
        `${baseUrl}/api/${version}/customers`,
      tickets:
        this.twentyConfigService.get('HVAC_TICKET_SERVICE_URL') ||
        `${baseUrl}/api/${version}/tickets`,
      equipment:
        this.twentyConfigService.get('HVAC_EQUIPMENT_SERVICE_URL') ||
        `${baseUrl}/api/${version}/equipment`,
      maintenance:
        this.twentyConfigService.get('HVAC_MAINTENANCE_SERVICE_URL') ||
        `${baseUrl}/api/${version}/maintenance`,
      semanticSearch:
        this.twentyConfigService.get('HVAC_SEMANTIC_SEARCH_URL') ||
  ...
  public getBooleanConfig(
    key: keyof ConfigVariables | string,
    defaultValue = false,
  ): boolean {
    let value: any;
