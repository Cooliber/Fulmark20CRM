...
import { HvacErrorHandlerService, HvacError, HvacErrorType, HvacErrorCategory } from '../services/hvac-error-handler.service';
import { ExceptionHandlerService } from 'src/engine/core-modules/exception-handler/exception-handler.service';
import { UserInputError, InternalServerError, ForbiddenError, NotFoundError } from 'src/engine/core-modules/graphql/utils/graphql-errors.util';
...
export class HvacExceptionFilter implements ExceptionFilter, GqlExceptionFilter {
  ...
  private handleGraphQLException(exception: HvacException, host: ArgumentsHost) {
    ...
    const hvacError: HvacError = {
      id: `hvac-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
      type: exception.type,
      category: exception.category,
      message: exception.message,
      polishMessage: this.getPolishErrorMessage(exception.type),
  ...
  private mapCategoryToSeverity(category: HvacErrorCategory): 'low' | 'medium' | 'high' | 'critical' {
    switch (category) {
      case HvacErrorCategory.SYSTEM:
      case HvacErrorCategory.EXTERNAL_SERVICE:
        return 'critical';
